...
class _TextPanelState extends State<TextPanel> {
  ...
  void _onSliderChanged(double value, Function(double) setter) {
    ...
    // For variable fonts, force cache invalidation without using Future.microtask
    if (_isVariableFont(_getCurrentFont())) {
      // Direct call to bust the TextOverlay cache
      TextOverlay.forceFontRefresh();
    }

    // Notify the parent widget once
    widget.onSettingsChanged(widget.settings);
  }
  ...
  static void _refreshPresets() {
    ...
    PresetRefreshService().refreshAspect(ShaderAspect.text);
  }
